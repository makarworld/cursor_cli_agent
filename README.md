...
**Команды бота:** `/start`, `/new`, `/status`, `/help`, `/set_prompt`, `/myprompt`, `/clear_prompt`, `/cd`, `/pwd`, `/ls`, `/mkdir`, `/cat`, `/rm`, `/self_fix`, `/bot_git_status`, `/bot_git_log`, `/bot_rollback`

**Самомодификация:** бот может править свой код (`telegram-bot/`), автоматически коммитить в git и перезапускаться. Подробнее — [DEPLOYMENT.md §10](DEPLOYMENT.md#10-самомодификация-бота).
